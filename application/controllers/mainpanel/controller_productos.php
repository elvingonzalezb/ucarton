...
class Controller_productos extends CI_Controller {
    ...
    private $current_section    ='categorias';
    private $section_catalogo   ='productos';    

    function __construct() {
        parent::__construct();
        $this->load->library('validacion');
        $this->load->model('mainpanel/Model_productos');
       $this->load->model('mainpanel/Model_categorias');        
    ...
    public function listado($id) {
        ...
        $listado            = $this->Model_productos->getLista($id);
    ...
    public function listar($padre=false) {
        ...
        //+------ LISTA DE CATEGORIAS --------+
        $categorias    = $this->Model_categorias->getLista(0);
        ...
        if($padre<>false){
            ...
            //+------ LISTA DE SUBCATEGORIAS --------+ 
            //+----- LISTA PRODUCTOS ---------+
            $listado            = $this->Model_productos->getLista($padre);
            $data["listado"]    = $listado;
            //+----- LISTA PRODUCTOS ---------+
             $data["padre"]     = $padre;
        }
    ...
    public function nueva() {
       ...
       // $ultOrden=$this->Model_productos->ultimo(0);
    ...
    public function grabar() 
    {
        ...
        else
        {
            ...
            redirect('mainpanel/productos/nuevo/'.$id);
        }

        $ultimo_id = $this->Model_productos->grabar($data);
        if($ultimo_id==true)
        {
            $this->session->set_userdata("success",'Se procesó correctamente la información');
            redirect('mainpanel/productos/listado/'.$padre);
        }  
        else
        {
    ...
    public function edit($id_producto) {
        ...
        $producto = $this->Model_productos->get($id_producto);
        $data['producto'] = $producto;
        $padre = $producto->id;
        $categoria = $this->Model_productos->getCategoria($padre);
        ...
        else
        {
            ...
            $subcategorias = $this->Model_categorias->getLista($categoria->padre);
        }
    ...
    public function actualizar()
    {
        ...
        if ( $this->my_upload->uploaded == true  )
        {
            ...
            $data['imagen'] = $this->my_upload->file_dst_name;
            $this->my_upload->clean();
            $this->Model_productos->updateFoto($data["imagen"], $imgatng, $id_foto);
        }
        $result=$this->Model_productos->update($id_foto, $data);
        if($result==true){
            $this->session->set_userdata("success",'Se procesó correctamente la información');
        }else{
        ...
        redirect('mainpanel/productos/edit/'.$id_foto);
    }

    public function borrar($id_foto) {
        $this->validacion->validacion_login();        
        //+----  BORRAMOS IMAGEN -------------+
        $foto      = $this->Model_productos->get($id_foto);
	    $imagen    = $foto->imagen;
        $id        = $foto->id;
        @unlink('files/productos/'.$imagen);
        //+------ BORRAMOS foto ---------+
        $resultado=$this->Model_productos->delete($id_foto); 
        //+------ BORRAMOS foto ---------+
        if($resultado==true)
        {
            $this->session->set_userdata("success",'Se procesó correctamente la información');
        ...
        redirect('mainpanel/productos/listado/'.$id);
    }
    ...
    public function destacado()
    {
        if ($_POST) {
            ...
            $updated = $this->Model_productos->update($id, $data);
            if($updated){
                $result['estado'] =  ($estado == 1)? '1' : '2';
                $result['msj'] = 'El estado se ha actualizado';
            }else{
                $result['estado'] = ($estado == 1)? '2' : '1';
